class Game {
  ...
  _updateCamera() {
    ...
    this.cameraProps.next = new THREE.Vector3(
      (pointA.x + pointB.x) / 2,
      0,
      (pointA.z + pointB.z) / 2
    );
  ...
  _updateCameraProps() {
    if (
      this.cameraProps.current.x > this.cameraProps.next.x ||
      this.cameraProps.current.z > this.cameraProps.next.z
    ) {
  ...
  _createCube() {
    ...
    if (this.cubes.length) {
      ...
      if (this.cubeStat.nextDir === "left") {
        cube.position.x =
          this.cubes[this.cubes.length - 1].position.x - Math.random() * 4 - 6;
      } else {
        cube.position.z =
          this.cubes[this.cubes.length - 1].position.z - Math.random() * 4 - 6;
      }
    }
